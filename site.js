...
(function () {
  ...
  /* ---------- Contact form ---------- */
  // Messages via Gmail go here (FormSubmit). X DMs go to @Retra_maps.
  var CONTACT_EMAIL = "okoiemmanuel715@gmail.com";
  var X_DM_BASE = "https://x.com/messages/compose?recipient_id=1871553112470949888";
  ...
  if (form) {
    ...
    var xButton = document.getElementById("contact-x");
    ...
    function showSuccess(via) {
      ...
      form.hidden = true;
      if (via === "x") {
        successBox.textContent = "Thanks — X should open with your message ready. Send the DM there and I will reply.";
      } else {
        successBox.textContent = "Thanks, your message has been sent to my Gmail. I will reply to the email address you gave.";
      }
      successBox.hidden = false;
      try { successBox.focus(); } catch (e) { /* ignore */ }
    }

    // Send via X: same validation, then open a DM to @Retra_maps with the message filled in.
    if (xButton) {
      xButton.addEventListener("click", function (event) {
        event.preventDefault();
        if (!validateForm()) { return; }
        openXMessage(buildMessageText());
        showSuccess("x");
      });
    }

    // Send via Gmail: posts to FormSubmit → okoiemmanuel715@gmail.com
    form.addEventListener("submit", function (event) {
      ...
      window.fetch("https://formsubmit.co/ajax/" + encodeURIComponent(CONTACT_EMAIL), {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          topic: topic,
          message: message,
          _subject: "Retra Maps contact: " + topic
        })
        ...
        errorBox.textContent = "Email could not be sent right now. Try “Send via X”, or try Gmail again in a moment.";
      }).then(function () {
        submitButton.disabled = false;
        submitButton.textContent = "Send via Gmail";
      });
    });
  }
})();
